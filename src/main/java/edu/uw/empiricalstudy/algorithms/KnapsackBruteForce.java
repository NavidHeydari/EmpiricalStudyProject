...
public class KnapsackBruteForce {
	...
	private int calculateBruteForce(int[] values, int[] weights, int capacity, int index) {
		...
		// far:"+capacity + ", indexAtThispoint:"+index);// + " val=" +
		// values[index] + " | weigth="+weights[index]);

		// base case
		if (capacity <= 0 || index < 0 || index == values.length) {
			// System.out.println("buttom rock! cap="+capacity + " |
			// index="+index);
	...
	public static void main(String[] args) {
		...
		int values[] = new int[] { 1, 9, 5, 2 };// {1,2,5,9};//{ 60, 100, 120 };
		int weights[] = new int[] { 2, 3, 3, 4 };// {2,3,3,4};//{ 10, 20, 30 };
		int capacity = 7;// 50;
